...
Nutshell.closeAllNutshells = () => {
  ...
  let allExpandables = [...document.querySelectorAll(".nutshell-expandable")],
    nestedExpandables = [
      ...document.querySelectorAll(".nutshell-expandable .nutshell-expandable"),
    ];
...
Nutshell.promisePurifiedHTMLFromURL = (url) => {
  ...
  return new Promise(async (resolvePurifiedHTML, rejectPurifiedHTML) => {
    ...
    if (_isWikipedia(url)) {
      ...
      if (found) {
        fetch(
          `https://${domain}.wikipedia.org/w/api.php?${_objectToURLParams(
            params
          )}`
        )
          .then((response) => response.json())
          .then((data) => {
            ...
            pageHTML = pageHTML.replace(
              /<span class="mw-editsection">.*?<\/span>/g,
              ""
            );
            // remove all links with title that starts with Edit Section
            pageHTML = pageHTML.replace(
              /<a.*?title="Edit section.*?<\/a>/g,
              ""
            );
            pageHTML = pageHTML.replace(
              /<span class="mw-editsection-bracket">.*?<\/span>/g,
              ""
            );
            ...
            pageHTML = pageHTML.replaceAll(
              /href="\/wiki/g,
              `href="https://${domain}.wikipedia.org/wiki`
            );
